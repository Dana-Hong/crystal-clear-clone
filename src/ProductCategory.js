...
export default function ProductCategory(props) {
    ...
    const productWidgetElements = productData.models.map(model => <ProductWidget key={nanoid()} img={model.img} name={model.name} path={model.path} />);
        ...
        <section className="bg-yellow-50 shadow-[inset_0_2px_20px_0_rgba(0,0,0,.3)]">
            <div className="max-w-screen-2xl mx-auto mb-40 px-6">
                ...
                <ul className="max-w-4xl mx-auto mb-14">
                    <p className="2xs:text-lg md:text-2xl mb-8">{productData.benefitsIntro}</p>
                    {listElements}
                </ul>
                <div className="
                    max-w-screen-2xl
