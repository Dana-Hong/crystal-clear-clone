    ...
    const cartListElements = cart.map(item => {
        return (
            <div key={nanoid()} className='border-2 border-gray-500 flex 2xs:flex-col 2xs:items-center md:flex-row md:justify-between md:items-center py-2 mb-4 rounded rounded-2xl'>
                ...
                <div className="border-2 border-red-300 md:mr-12 flex flex-col items-center">
                    <div className="flex mx-auto w-60 justify-between border-2 border-green-300">
                        <p className='2xs:p-4 md:px-0 md:text-xl w-[125px] text-center border-2'>
                            ...
                            <input 
                                ...
                                className="border-2 w-1/2 placeholder:text-slate-800 pl-4 mt-2 border"
                                onChange={(event) => handleChange(event, item)}
                                value={getCurrentQuantity(item.name)}
                                >    
                            </input>
                        </p>
                        <p className='2xs:p-4 md:px-0 md:text-xl w-[125px] text-center border-2'><span className="md:hidden font-bold">Price:</span> {`$${item.price}`}</p>
                    </div>
                    <button 
                ...
                <div className="md:flex md:justify-between py-2">
                    ...
                    {cartQuantity !== 0 &&
                        <div className="flex mr-12 w-60 justify-between">
                            <p className="2xs:hidden md:block text-xl w-[125px] text-center font-bold">Quantity</p>
                            <p className="2xs:hidden md:block text-xl w-[125px] text-center font-bold">Price</p>
                        </div>
                    }
                </div>
                {cartListElements}
